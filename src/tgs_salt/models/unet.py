...
from .model_utils import scSE_block, attention_gating_block, deep_supervision, hypercolumn
...
def vanilla_unet(input_shape, random_state):
    ...
    conv_aux3         = deep_supervision(bn_15, 128, 'aux3', activation, initializer)
    conv_aux3_score   = Activation('sigmoid', name='conv_aux3_score') (conv_aux3)
    ...
    conv_aux2         = deep_supervision(bn_18, 64, 'aux2', activation, initializer)
    conv_aux2_score   = Activation('sigmoid', name='conv_aux2_score') (conv_aux2)
    ...
    conv_aux1         = deep_supervision(bn_21, 32, 'aux1', activation, initializer)
    conv_aux1_score   = Activation('sigmoid', name='conv_aux1_score') (conv_aux1)
    ...
    hyp_u0d           = hypercolumn(bn_24, bn_30, bn_28, bn_26)
    bn_31             = BatchNormalization(name='bn_31') (hyp_u0d)
    conv_u0d_score    = Conv2D(1, (1, 1), padding='same', activation='sigmoid', name='conv_u0d-score', kernel_initializer=initializer) (bn_31)


    model = Model(
        inputs=inputs,
        outputs=[conv_aux3_score, conv_aux2_score, conv_aux1_score, conv_u0d_score],
        name='vanilla_unet'
    )
